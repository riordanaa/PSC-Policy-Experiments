...
Compares DRL performance when the DS agent observes different subsets of metrics.
Compares DRL performance with different subsets of observable metrics.

Usage:
  python study_state_space.py --fast
  python study_state_space.py --episodes 80 --periods 300
  python study_state_space.py --episodes 80 --seeds 42,123,256
"""
...
STATE_CONFIGS = {
    'minimal': {
        'DS_STATE_METRICS': ['Backlog', 'Inventory-Level', 'Demand'],
        'HC_STATE_METRICS': ['Backlog'],
        'MN_STATE_METRICS': ['Demand'],
    },
    'partial': {
    ...
    'full': {
        'DS_STATE_METRICS': ['Backlog', 'Inventory-Level', 'Order', 'Delivery',
                             'Inventory', 'On-Order', 'Demand', 'Lead-time', 'Up-to-level'],
        'HC_STATE_METRICS': ['Inventory', 'Demand', 'Loss', 'Backlog', 'Order',
                             'Up-to-level', 'Lead-time', 'On-Order'],
        'MN_STATE_METRICS': ['Inventory', 'Backlog', 'In production', 'Up-to-level',
                             'Demand', 'Lead-time'],
    },
}


def _apply_state_config(name):
    sc = STATE_CONFIGS[name]
    config.DS_STATE_METRICS = list(sc['DS_STATE_METRICS'])
    config.HC_STATE_METRICS = list(sc['HC_STATE_METRICS'])
    config.MN_STATE_METRICS = list(sc['MN_STATE_METRICS'])


def _run_drl(episodes, periods, state_cfg_name, seed):
    _apply_state_config(state_cfg_name)
    config.set_global_seeds(seed)
    tf.keras.backend.clear_session()

    profile = ConfigDrivenProfile(f'study2_{state_cfg_name}')
    ...
    state_dim = drl_dms[0].ds_env.state_size if drl_dms else 0
    print(f'    [{state_cfg_name}] state_dim={state_dim}')
    ...
    for eps in range(episodes):
        ...
        print(f'    [{state_cfg_name}] Ep {eps:3d}/{episodes}: '
...
def run_study(episodes, periods, seeds):
    ...
    for seed in seeds:
        ...
        for cfg_name in STATE_CONFIGS:
            tag = f'{cfg_name}_seed{seed}'
            print(f'\n  --- {cfg_name} state config (seed {seed}) ---')
            drl_eps, curve = _run_drl(episodes, periods, cfg_name, seed)
            last_ep = episodes - 1
            ...
            print(f'\n  Results: {cfg_name} (ep {last_ep}) vs Baseline')
            results = {}
            for phase, (s, e) in [('pre', pre), ('during', during), ('post', post)]:
                ...
                r = print_comparison(b, d, f'{cfg_name} {phase.upper()} ({s}-{e})')
                results[phase] = {k: float(v) for k, v in r.items()}

            all_results[tag] = {
                'state_config': cfg_name,
                'seed': seed,
                'state_dim': curve[0].get('state_dim', '?') if curve else '?',
                'results': results,
    ...
    # Summary
    print(f'\n{"="*90}')
    print(f'  STUDY 2 SUMMARY: State-Space Sensitivity')
    print(f'  {len(seeds)} seed(s), {episodes} eps x {periods} periods')
    print(f'{"="*90}')

    for phase in ['pre', 'during', 'post']:
        print(f'\n  --- {phase.upper()} ---')
        print(f'  {"Config":<10} | {"Profit Diff":>12} | {"Sys Bklg Diff":>13} | {"Verdict":>8}')
        print(f'  {"-"*10}-+-{"-"*12}-+-{"-"*13}-+-{"-"*8}')
        for cfg_name in STATE_CONFIGS:
            pd_vals = [all_results[f'{cfg_name}_seed{s}']['results'][phase]['profit_diff']
                       for s in seeds]
            sb_vals = [all_results[f'{cfg_name}_seed{s}']['results'][phase]['sys_bklg_diff']
                       for s in seeds]
            pd_m, sb_m = np.mean(pd_vals), np.mean(sb_vals)
            verdict = ('PASS' if pd_m > 0 and sb_m < 0
                       else 'PARTIAL' if pd_m > 0 or sb_m < 0 else 'FAIL')
            print(f'  {cfg_name:<10} | {pd_m:+12.1f} | {sb_m:+13.1f} | {verdict:>8}')

    out_dir = os.path.join('checkpoints', 'study2_state_space')
...
def main():
    p = argparse.ArgumentParser(description='Study 2: State-space sensitivity')
    ...
    print('=' * 90)
    print('  STUDY 2: State-Space Sensitivity')
    print(f'  Episodes: {episodes}  Periods: {periods}  Seeds: {seeds}')
    print(f'  Configs: {list(STATE_CONFIGS.keys())}')
    print('=' * 90)
