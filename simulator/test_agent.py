...
from simulator.agent import Agent, HealthCenter, Manufacturer, Distributor, Item
...
class TestAgent(unittest.TestCase):
    ...
    def test_receive_delivery(self):
        ...
        upstream_agent1.id = 10
        upstream_agent2 = Agent()
        upstream_agent2.id = 20
        agent = Agent()

        order1 = Order()
        order1.amount = 10
        order1.src = agent.name()
        order1.dst = upstream_agent1.name()
        ...
        order2 = Order()
        order2.amount = 20
        order2.src = agent.name()
        order2.dst = upstream_agent2.name()
        ...
        order3 = Order()
        order3.amount = 30
        order3.src = agent.name()
        order3.place_time = 12
        order3.dst = upstream_agent1.name()
        agent.on_order.append(order3)
        agent.get_history_item(12)['order'].append(copy.copy(order3))

        item = Item()
        item.amount = 20
        agent.receive_delivery(item, upstream_agent1.name(), 20)
    ...
    def test_make_order(self):
        ...
        order = agent.make_order(dst.name(), 100, 10)

        self.assertEqual(order.place_time, 10)
        self.assertEqual(order.amount, 100)
        self.assertEqual(order.src, agent.name())
        self.assertEqual(order.dst, dst.name())
    ...
    def test_order_unique_ids(self):
        """Verify that each Order gets a unique ID."""
        o1 = Order()
        o2 = Order()
        o3 = Order()
        self.assertNotEqual(o1.id, o2.id)
        self.assertNotEqual(o2.id, o3.id)
        self.assertNotEqual(o1.id, o3.id)

    def test_inventory_level(self):
        agent = Agent()
        self.assertEqual(agent.inventory_level(), 0)
        ...
        self.assertEqual(agent.inventory_level(), 80)

    def test_backlog_level(self):
        agent = Agent()
        self.assertEqual(agent.backlog_level(), 0)

        o1 = Order()
        o1.amount = 15
        agent.backlog.append(o1)

        o2 = Order()
        o2.amount = 25
        agent.backlog.append(o2)

        self.assertEqual(agent.backlog_level(), 40)
...
class TestManufacturer(unittest.TestCase):
    ...
    def test_apply_in_production_in_history(self):
        ...
        self.assertEqual(len(manufacturer.get_history_item(10)['production']), 1)
